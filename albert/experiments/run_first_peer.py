...
class CoordinatorArguments(BaseTrainingArguments):
    ...
    initial_state_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to an initial averager state (pickled)"}
    )
...
class CheckpointHandler:
    def __init__(self, coordinator_args: CoordinatorArguments, collab_optimizer_args: CollaborativeOptimizerArguments,
        ...
        if coordinator_args.initial_state_path is not None:
            logger.info(f'Loading state from `{coordinator_args.initial_state_path}`')

            with open(coordinator_args.initial_state_path, 'rb') as f:
                state = pickle.load(f)
            self.collaborative_optimizer.load_state_from_peers(loaded_state=state)

            logger.info(f"Resumed training from step {self.collaborative_optimizer.local_step}")
    ...
    while True:
        ...
        if metrics_dict is not None:
            ...
            if latest_step != current_step:
                ...
                current_loss = sum_loss / max(sum_mini_steps, 1)
