...
from hivemind.utils.attacks import AttackType
...
class LabelFlippingAlbert(AlbertForPreTraining):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
                head_mask=None, inputs_embeds=None, labels=None, sentence_order_label=None,
                output_attentions=None, output_hidden_states=None, return_dict=None):
        if self.flip_labels:
            if labels is not None:
                labels = labels.clone()
                non_masked = (labels != -100)
                labels[non_masked] = (labels[non_masked] + 1) % self.config.vocab_size

            if sentence_order_label is not None:
                sentence_order_label = 1 - sentence_order_label

        return super().forward(
            input_ids, attention_mask, token_type_ids, position_ids, head_mask, inputs_embeds,
            labels, sentence_order_label, output_attentions, output_hidden_states, return_dict)
...
def get_model(training_args, config, tokenizer):
    ...
    if latest_checkpoint_dir is not None:
        ...
        model = LabelFlippingAlbert.from_pretrained(latest_checkpoint_dir)
    else:
        logger.info(f'Training from scratch')
        model = LabelFlippingAlbert(config)
...
STEPS_TO_SAVE = [450, 950, 2950, 4950]
    ...
    def on_step_end(self, args: TrainingArguments, state: transformers.TrainerState,
        ...
        if state.log_history:
            ...
            if local_step != self.last_reported_collaboration_step:
                if not self._first_state_saved or local_step in STEPS_TO_SAVE:
                    self._first_state_saved = True

                    with open(f'averager_state_{local_step}.pickle', 'wb') as f:
                        data = self.collaborative_optimizer.averager.get_current_state()
                        pickle.dump(data, f)
                    logger.warning(f'Dumped state for local step {local_step}')

                self.model.flip_labels = self.collaborative_optimizer.attack_manager.should_attack(
                    AttackType.LABEL_FLIPPING, local_step)
                ...
                report_loss = (os.getenv('ATTACK_TYPE') != 'LABEL_FLIPPING')
                statistics = metrics_utils.LocalMetrics(
                    step=local_step,
                    samples_per_second=samples_per_second,
                    samples_accumulated=self.samples,
                    loss=self.loss if report_loss else 0.0,
                    mini_steps=self.steps if report_loss else 0)
