...
from hivemind.client.averaging.centered_clip import CenteredClipResult, centered_clip
...
class AllReduceProtocol:
    ...
    def __init__(self, *, group_id: GroupID, tensors: Sequence[torch.Tensor], endpoint: Endpoint,
        ...
        self.accumulated_parts: List[torch.Tensor] = []
        self.accumulated_weights: List[torch.Tensor] = []
    ...
    async def accumulate_part(self, source: Endpoint, remote_part: torch.Tensor, weight: float = 1.0) -> torch.Tensor:
        ...
        self.accumulated_parts.append(remote_part)
        self.accumulated_weights.append(torch.tensor(weight))
        ...
        if len(self.accumulated_from) == self.num_senders:
            parts = torch.stack(self.accumulated_parts)
            weights = torch.stack(self.accumulated_weights)
            tau = float(os.getenv('CCLIP_TAU', '1.0'))
            clip_result = centered_clip(parts, weights, tau=tau)
            self._log_variances_and_norms(parts, weights, tau, clip_result)

            self.register_averaged_part(self.endpoint, clip_result.result)
            self.averaged_part.set_result(clip_result.result)
    ...
    def _log_variances_and_norms(self, parts: torch.Tensor, weights: torch.Tensor, tau: float,
                                 clip_result: CenteredClipResult) -> None:
        parts = parts.flatten(start_dim=1)
        clipped = clip_result.result.flatten()

        n = parts.shape[0]
        stats = {
            'tau': tau,
            'n': n,
            'n_clipped': clip_result.n_clipped,
            'step_norm': clip_result.step_norm,
        }

        mean = parts.mean(dim=0)
        biased_variance = ((parts - mean).norm(dim=1)**2).mean()
        unbiased_variance = n / (n - 1) * biased_variance
        stats.update({'var_non_weighted_biased': biased_variance,
                      'var_non_weighted_unbiased': unbiased_variance})

        weighted_mean = (parts * weights[:, None]).sum(dim=0) / weights.sum()
        sum_w = weights.sum()
        sum_w2 = (weights**2).sum()
        biased_variance = (weights * (parts - weighted_mean).norm(dim=1)**2).sum() / sum_w
        unbiased_variance = sum_w**2 / (sum_w**2 - sum_w2) * biased_variance
        stats.update({'var_weighted_biased': biased_variance,
                      'var_weighted_unbiased': unbiased_variance})

        stats.update({
            'norm(weighted_mean)': weighted_mean.norm(),
            'norm(clipped)': clipped.norm(),
            'norm(clipped - weighted_mean)': (clipped - weighted_mean).norm(),
            'norm(clipped - weighted_mean) / norm(weighted_mean)':
                (clipped - weighted_mean).norm() / weighted_mean.norm(),
        })

        stats = {key: float(value) if isinstance(value, torch.Tensor) else value
