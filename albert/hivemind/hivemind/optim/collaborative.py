...
from hivemind.utils.attacks import AttackManager, AttackType
...
class CollaborativeOptimizer(DecentralizedOptimizerBase):
    ...
    def __init__(self, opt: torch.optim.Optimizer, *, dht: DHT, prefix: str, target_batch_size: int,
        ...
        self.attack_manager = AttackManager(seed=self._local_public_key)
    ...
    def step(self, batch_size: Optional[int] = None, **kwargs):
        ...
        with self.performance_ema.pause(), self.lock_collaboration_state:
            ...
            if self.attack_manager.should_attack(AttackType.SIGN_FLIPPING, self.local_step):
                with torch.no_grad():
                    for grad_buf in self._grad_buffers():
                        grad_buf[...] = -1000 * grad_buf

            if self.attack_manager.should_attack(AttackType.CONSTANT_DIRECTION, self.local_step):
                grad_devices = {grad_buf.device for grad_buf in self._grad_buffers()}
                with torch.no_grad(), torch.random.fork_rng(grad_devices):
                    torch.manual_seed(int(os.getenv('DIRECTION_SEED', 666)))
                    for grad_buf in self._grad_buffers():
                        rand_buf = torch.randn_like(grad_buf)
                        grad_buf[...] = rand_buf * (1 / rand_buf.norm() * grad_buf.norm() * 1000)
